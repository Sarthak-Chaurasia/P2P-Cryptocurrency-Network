...
T_tx = 1.0e-3 if len(sys.argv) < 5 or sys.argv[4] == -1 else float(sys.argv[4])  # mean inter-arrival time between transactions
T_interarrival = 10.0e-3 if len(sys.argv) < 6 or sys.argv[5] == -1 else float(sys.argv[5])  # mean inter-arrival time between blocks
txn_blk_ratio = T_tx/T_interarrival if len(sys.argv) < 7 or sys.argv[6] == -1 else float(sys.argv[6])  # ratio of txn to block inter-arrival times
total_txn_blks = 1000 if len(sys.argv) < 8 or sys.argv[7] == -1 else int(sys.argv[7])  # total number of transactions and blocks to be generated
